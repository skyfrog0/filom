...
Filom 是一个基于 **Koa 2** 的单文件后端 + 纯前端单页应用，同时提供文件管理、实时聊天室和大文件断点续传功能。
...
2. **REST API 路由** — Koa Router 处理文件操作：
   ...
   - 分片上传：`POST /api/upload/init`、`GET /api/upload/:uploadId/status`、`POST /api/upload/:uploadId/chunk`、`POST /api/upload/:uploadId/merge`
...
- 分片大小滑块（10/25/50/100 MB，默认 100 MB）和并发数下拉（1/2/3/5/8/10，默认 2）
